<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!doctype html>
<html lang="en">
<head>
	<meta charset="UTF-8" />
	<title>예매 페이지</title>
	<style>
		@IMPORT url("<%=request.getContextPath()%>/css/reservation/reservationPage.css");
	</style>
</head>
<body>
<jsp:include page="../main/header.jsp"/>
	
	<form action="<%=request.getContextPath()%>/reservation/selection.do">
	<div class="rdiv" id="clear">
	<fieldset style="margin-left: 141px; margin-top: -239px;">
		<legend> [RESERVATION] </legend>
		<table class="reserveTable" summary="예매테이블">
		<colgroup> <!-- column 한 셀에 대한 가로값을 입력 -->
			<col width="30%"/>
			<col width="15%"/>
			<col width="35%"/>
			<col width="20%"/>
		</colgroup>
		<thead > <!-- table head : 테이블 속 컬럼명을 지정 -->
			<tr >
				<th>영화</th>
				<th>극장</th>
				<th>날짜</th>
				<th>시간</th>
			</tr>
		</thead>
		<tbody class="tbodyOption"> 
			<tr>
				<td>
					<select name="movie">
					  <option value="jurassicWorld">쥬라기공원</option>
					  <option value="madMax">매드맥스</option>
					  <option value="sanAndreas">샌안드레아스</option>
					</select>
				</td>
				<td>
					<select class="theaterSelect" name="theater">
					  <option value="ATheater">A관</option>
					  <option value="BTheater">B관</option>
					  <option value="CTheater">C관</option>
					  <option value="DTheater">D관</option>
					  
					</select>
				</td>
				<td>
					<input type="date" id="date" name="date" min="2015-06-19" max="2015-06-26"/>
				</td>
				<td>
					<select class="timeSelect" name="time" >
					  <option value="10:00">10:00</option>
					  <option value="12:00">12:00</option>
					  <option value="15:00">15:00</option>
					</select>
				</td>
			</tr>
		</tbody>
	</table>
	<p>영화, 극장, 날짜, 시간을 선택하셨으면 >>> <input type="submit" value="선 택" /></p>
	
	</fieldset>
	</div>
	</form>
	
</body>
<footer>
	<hr />
	<div>
		<pre> 회사소개 : 시들어 그들은 행복스럽고 힘있다. 풍부하게 열락의 보이는 앞이 힘차게 심장은 눈에 듣는다.
		 얼마나 트고, 무엇을 사랑의 힘차게 봄바람이다. 그들에게 위하여 보이는 과실이 유소년에게서 듣는다. 무엇을 가치를 뭇 군영과 몸이 같지 있다.
		</pre>
	</div>
</footer>
</html>